...
/* eslint-disable */ 
import { CoefficientOfDrag } from 'drag-model';
 ...
 */
export class EnvironmentalFactors {
...
//
export class PointMassBallisticSolver {
    constructor() {
        this.STEP = 0.075
    }

    /**
     * Calculates the pressure for different height and temperature.
     *   @param {Number} temperature is in Celsius
     *   @param {Number} height is in metres above sea level.
     *   @param {Number} pressure is the sea level pressure in hPa.
     * @returns the adjusted pressure.
     */
    pressureCorrection(temperature, height, pressure) {
     ...
     */
    getZeroCrossingLinear(x0, y0, x1, y1) {
        return x0 - y0 * (x1 - x0) / (y1 - y0);
    }

    /**
     * Sets the ballistic model to use.
     *   @param {string} system is either 'G1' or 'G7'
     */
    setSystem(system) {
        // console.log("Setting ballistic system to " + system)
        // this.CDTable = (system === 'G7' ? CD_G7 : CD_G1);
        try {
            this.getCD = new CoefficientOfDrag(system);
        } catch(e) {
            console.log(e);
        }
    ...
    */
     ...
     *   @param {Number} range       is the maximum range to calculate (in metres)
     *   @param {Number} angle       is the the launch angle relative to horizontal, positive up (in radians).
     *   @param {Number} velocity    is the initial velocity in metres / second.
     *   @param {Number} mass        is the mass of the projectile in kilogrammes.
     *   @param {Number} bc          is the the ballistic coefficient of the projectile in the selected system.
     *   @param {Number} system      is the use G1 or G7, strings 'G1' or 'G7' respectively.
     *   @param {Number} reference   is the the height of the horizontal reference.
     *   @param {EnvironmentalFactors} environment is the environmental factors to consider.
     ...
     */
    getTrajectory(range, angle, velocity, mass, bc, system, reference, environment) {
        ...
        while (s_x < range + this.STEP) {
            let dt = this.STEP / v_x;
     ...
     */
    getCrossings(trajectory, offset) {
        let crossings = { near: 0, far: 0 };
        for (let i = 0; i < trajectory.length - 2; i++) {
            if (trajectory[i].y >= offset && trajectory[i + 1].y < offset) {
                crossings.far = this.getZeroCrossingLinear(trajectory[i].x, trajectory[i].y - offset, trajectory[i + 1].x, trajectory[i + 1].y - offset);
                return crossings; // can't go back up.
            }
            if (trajectory[i].y <= offset && trajectory[i + 1].y > offset) {
                crossings.near = this.getZeroCrossingLinear(trajectory[i].x, trajectory[i].y - offset, trajectory[i + 1].x, trajectory[i + 1].y - offset);
            }
        }
        return crossings;
    }
     ...
     */
    linearInterpolate(x, first, second) {
     ...
     *   @param {Number} range       is the zero range in metres.
     *   @param {Number} offset      is the desired vertical offset of the intersection of the trajectory at the zero range.
     *   @param {Number} velocity    is the initial projectile velocity in metres/second.
     *   @param {Number} mass        is the mass of the projectile in kg.
     *   @param {Number} bc          is the the ballistic coefficient in the active system.
     *   @param {Number} reference   is the the height of the horizontal reference.
     *   @param {EnvironmentalFactors} environment is the environmental factors to consider.
     * @returns the angle in radians.
     */
    getZeroingAngle(range, offset, velocity, mass, bc, system, reference, environment) {
    ...
    getEnvelope(trajectory) {
        ...
        return {
            maxPoint: maxPoint,
            pbr: this.getCrossings(trajectory, -maxPoint.y),
            zeros: this.getCrossings(trajectory, 0)
        };
    }
